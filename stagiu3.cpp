...
///BULLETS SPAWN
    ...
    boss.x=mijx+1;
    boss.y=200;
    /* coordonatele centrului boss-ului*/
    int r_boss=125;
    /* raza boss-ului*/
    int sh_r=5;
    /* numarul de straturi ale scutului*/
    int boss_shieldr=150;
    /* raza scutului boss-ului*/
    int r_eye=30;
    /* raza ochiului*/
    int dis_eye=70;
    /* distantadintre ochi*/
    int eyex=mijx-30;
    int eyey=150;
    /* coordonatele centrului, primului ochi*/
    int r_sad=50;
    /* raza gurii*/
    int sady=250;
    /* coordonata de inceput a gurii*/
    int dir_boss=1;
    /* directia de deplasare a boss-ului*/
    int nr_shots=0;
    /* numarul de lovituri aplicate boss-ului*/
    int dif_shots=0;
    /* vechea valoare a nr_shots in cazul unei noi lovituri*/
    int life_boss=lungime_ecran;
    /* viata boss-ului*/
    int damage=100;
    /* pagubele create de o singura lovitura de tun*/
    for(int i=0; i<=7; i++)
        circle(boss.x,boss.y,r_boss-i);
    /* se deseneaza boss-ul*/
    ...
    while(life_boss>0&&life_player>=0)
    {
        ...
        /* daca boss-ul se afla in extrimitatea din dreapta a ecranului atunci se schimba directia*/
        ...
        /* deplasarea boss-ului catre dreapta, respectiv stanga*/
        ...
        for(int i=1; i<=5; i++)
            ...
            /* se deseneaza boss-ul (ochii, gura si restul corpului)*/

        ///BOSS SHIELD
        if(sh_r)
        {/*daca nu s-au dus toate straturile din scutul boss-ului*/
            int cont=1;
            setcolor(WHITE);
            for(int i=0; i<sh_r; i++)
                circle(boss.x,boss.y,boss_shieldr-i);
                /*afiseaza cate straturi au mai ramas*/
            ...
            /* daca tunul este activ si boss-ul se afla in aceasta raza
            atunci se verifica daca nu se face o confuzie dintre lovituri,
             adica o lovitura sa nu ii scada 2 staturi deodata, in caz afirmativ,
              se deuce un strat, iar cand numarul de straturi ajunge la 0, acestea dispar, altfel
              scutul boss-ului ar scadea la 0 dupa prima lovitura*/
        if(time_add>=current_time)
        {
            setcolor(YELLOW);
            outtextxy(300,mijy,"THE BOSS SHIELD IS DOWN !");
        }
        /* daca scutul este distrus, se afiseaza un mesaj corespunzator pe ecran*/
        ///LIFE BOSS
        ...
        /* daca tunul este activ si boss-ul se afla in aceasta raza
            atunci se verifica daca nu se face o confuzie dintre lovituri,
             adica o lovitura sa nu aibe echivalentul a doua lovituri, altfel
             viata boss-ului ar scadea la 0 dupa prima lovitura*/
        if(sh_r<=0)
        {
            setfillstyle(SOLID_FILL,RED);
            bar(0,25,life_boss,35);
        }
        /*daca scutul este distrus, se afiseaza viata boss-ului*/
        /// FIRE CONTROL
    ...
    /* in rest toata mecanica si comenzile sunt aceleasi ca si la stagiul 1 si 2*/
    return 0;
}
